  ...
  <div class="py-32 animate-in fade-in max-w-7xl mx-auto px-6 text-left">
      ...
      <div class="max-w-4xl bg-slate-50 border border-slate-100 p-10 rounded-[2.5rem] flex flex-col md:flex-row gap-8 items-center ">
        ...
        <div class="space-y-2 text-center md:text-left">
          ...
          <div class="flex flex-wrap justify-center md:justify-start gap-6">
            <div v-for="(p, pi) in hero.points" :key="pi" class="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-slate-500 ">
    ...
    <div v-if="prestations && prestations.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-12 mb-32">
       <div v-for="(p, i) in prestations" :key="i" class="relative group bg-white border border-slate-100 rounded-[3rem] p-12 hover:shadow-2xl transition-all duration-700 flex flex-col space-y-8 animate-in slide-in-from-bottom-8 fade-in" :style="{ animationDelay: `${i * 100}ms` }">
          <div v-if="p.badge" class="absolute top-8 right-8 text-[9px] font-black uppercase tracking-widest bg-primary/10 text-secondary px-3 py-1 rounded-full">{{ p.badge }}</div>
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-baseline gap-2">
               ...
               <span class="text-sm font-bold text-slate-400 uppercase tracking-widest">{{ p.unit }}</span>
            </div>
          </div>
          ...
          <button @click="addToCart(p)" class="w-full py-5 rounded-2xl bg-slate-50 border border-slate-100 text-[11px] font-black uppercase tracking-[0.3em] text-primary group-hover:bg-primary group-hover:text-white transition-all ">
