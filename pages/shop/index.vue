    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 text-left">
      <NuxtLink :to="`/shop/${p.id}`" v-for="(p, idx) in products" :key="p.id" class="group bg-white rounded-[3rem] overflow-hidden hover:shadow-2xl transition-all duration-700 flex flex-col border border-slate-50 animate-in slide-in-from-bottom-8 text-left cursor-pointer" :style="{ animationDelay: `${idx * 100}ms` }">
        <div class="relative aspect-[4/5] overflow-hidden bg-[#F8FAFC] text-left">
          ...
          <div v-if="p.promo" class="absolute top-8 left-8 bg-primary text-primary px-5 py-1.5 rounded-full text-[10px] font-black tracking-[0.3em] shadow-xl">Offre Privilège</div>
        </div>
        <div class="p-10 flex flex-col flex-1 text-center space-y-6">
          <span class="text-[9px] font-black tracking-[0.5em] text-primary text-center">{{ p.category }}</span>
          <h3 class="text-xl font-bold text-primary tracking-tighter leading-snug min-h-[3.5rem] flex items-center justify-center text-center">{{ p.name }}</h3>
          ...
          <button @click.prevent="addToCart(p)" class="w-full bg-slate-50 border border-slate-100 py-6 rounded-3xl text-[11px] font-black tracking-[0.4em] text-primary hover:bg-secondary hover:text-white transition-all shadow-sm flex items-center justify-center gap-4 group/btn active:scale-95">
